...
def get_chatgpt_response(history: list, new_input: str, tools, tool_call_count=0):
    ...
    messages = trim_history(history) + [{"role": "user", "content": new_input}]
    ...
    if tool_calls_acc and tool_call_count < MAX_TOOL_CALLS:
        ...
        return get_chatgpt_response(messages + [assistant_msg] + tool_messages, "", tools, tool_call_count + 1)

    new_history = messages + [{"role": "assistant", "content": full_text}]

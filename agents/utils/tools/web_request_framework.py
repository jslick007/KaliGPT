...
def web_request_analysis(
    url: str, method: str = "GET", payload: str = None, headers: str = None, timeout: int = 10
) -> dict:
    """
    ...
    try:
        ...
        response = requests.request(
            method=method.upper(), url=url, data=payload, headers=req_headers, timeout=timeout, allow_redirects=True
        )
        ...
        analysis["security"]["cors"] = {"acao": acao, "acac": acac, "acam": acam}
        ...
        if acao and "null" in acao:
            analysis["security"]["findings"].append("WARNING: CORS allows 'null' origin - Potential sandbox bypass")
        ...
        analysis["security"]["caching"] = {"cache_control": cache_control, "pragma": pragma, "etag": etag}
        ...
        if response.headers.get("Content-Type", "").startswith("text/html"):
            ...
            if "charset" not in response.text:
                analysis["security"]["findings"].append(
                    "HTML response missing charset specification - May lead to encoding issues"
                )
        ...
        if analysis["security"]["is_https"]:
            ...
            analysis["security"]["findings"].append("Site uses HTTP - Consider upgrading to HTTPS for security")

        return analysis

    except requests.exceptions.RequestException as e:
        return {"error": str(e), "request": {"url": url, "method": method.upper()}}
...
def get_raw_response(
    ...
    try:
        ...
        # Finalize response
        return {
            "request": {"method": method.upper(), "url": url, "headers": req_headers, "body": payload},
            "response": {
